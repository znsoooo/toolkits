...
def data2qrs(file, rect, mat):
    ...
    with open(file, 'rb') as f:
        while 1:
            ...
            page = np.array([[string2qr(cell_string, rect) for cell_string in row] for row in page_data])
            page2 = np.concatenate(np.concatenate(page, axis=1), axis=1)
            page3 = expand(page2, int((1080-page2.shape[0])/2), int((1920-page2.shape[1])/2))
...
data2qrs('passage2019.rar', (100, 100), (4, 8))
